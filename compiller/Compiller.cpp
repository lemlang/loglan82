...
bool Compiller::OnInit() {
    ...
    file.SetExt("ccd");
    wxString filenameCcd = file.GetFullPath();
    ...
    temporaryDirectory.SetExt("ccd");
    wxString temporaryCcd = temporaryDirectory.GetFullPath();
        ...
        if (i != -1) { // get include file
            ...
            if ((i != -1) && (j != -1)) {
                ...
                fileToInclude.sprintf("%s%s%s", file.GetPath(wxPATH_GET_VOLUME & wxPATH_GET_SEPARATOR),
                                      wxFileName::GetPathSeparators(), fileNameToInclude );
                ...
                while (!fileToIncludeStream.Eof()) {
                    ...
                    comps << readedLineOfInclude << endl;
                    includeFile->len++;
                    currentline++;
                }
            ...
            comps << readedLine.ToAscii() << endl;
        }
        currentline++;
    }// eof
    wxLogError("Compiller working");
    compfile.Close();
    ...
    if (wxFile::Exists(temporaryLcd.ToAscii())) {
        ...
        wxCopyFile(temporaryCcd, filenameCcd);
